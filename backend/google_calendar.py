...
class BeginFlow(GoogleAccountLabel,FlowClient):pass
...
class GoogleCalendars:
    ...
    def settings(self):
        with self.lock:
            ...
            return {'revision':self.config.revision,'client_id':self.config.client_id,'redirect_uri':self.config.redirect_uri,
                    ...
                    'accounts':[{'id':a['id'],'label':a['label'],'calendar_count':len(a['calendars'])} for a in self.accounts]}
    ...
    def begin(self,label,principal,client):
        with self.lock:
            ...
            self.flows[identifier]={'state':hashlib.sha256(state.encode()).hexdigest(),'verifier':verifier,'principal':principal,
                'client':hashlib.sha256(client.encode()).hexdigest(),'label':label,'revision':self.config.revision,'expires':self.clock()+600,'status':'waiting'}
            query={'client_id':self.config.client_id,'redirect_uri':self.config.redirect_uri,'response_type':'code',
                   'scope':' '.join(READ_SCOPES),'state':state,'code_challenge':challenge,'code_challenge_method':'S256',
    ...
    def callback(self,state,code,error):
        ...
        try:
            ...
            if (not self.valid_token(tokens.get('access_token')) or not self.valid_token(tokens.get('refresh_token'))
                or not set(READ_SCOPES)<=set(str(tokens.get('scope','')).split())):
    ...
    def finish(self,identifier,principal,client):
        with self.lock:
            ...
            account={'id':secrets.token_hex(16),'label':flow['label'],'refresh_token':flow['tokens']['refresh_token'],'calendars':[]}
    ...
    def catalog(self):
        with self.lock:
            ...
            return [{'entity_id':c['entity_id'],'kind':'calendar','name':c['name']+' · '+a['label'],
                     'can_create':False,'can_edit':False,'can_delete':False,'can_detect_presence':False,
    ...
    def events(self,entity,since,until):
        ...
        for item in result['items']:
            ...
            # Read-only until provider-specific edit/review is implemented. IDs
            # are withheld so HA's writer cannot act on Google event identities.
            rows.append({'summary':item.get('summary','Untitled event'),'description':item.get('description',''),
                         'location':item.get('location',''),'start':start,'end':end,
                         'rrule':'GOOGLE_RECURRING' if item.get('recurringEventId') else None})
        return rows
...
def install(app,google,authorize,owner):
    ...
    def begin(body:BeginFlow,principal=Depends(authorize)):return call(lambda:google.begin(body.label,principal,body.client))
