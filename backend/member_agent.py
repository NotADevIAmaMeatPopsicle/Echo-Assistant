...
from .member_hermes import MemberHermes, PersonalProvider, PersonalCancellation
...
class MemberSettings:
    ...
    def snapshot(self):
        ...
        with self.members.lock:
            ...
            personality=PERSONALITY+' This is a personal conversation. Saved facts belong only to this account. Never claim access to household lists, files, calendars or household Hermes tools.'
...
class MemberAgents:
    def __init__(self,members,store,provider,home,assistant,*,household_runtime=None):
        self.members,self.store,self.provider,self.home,self.assistant=members,store,provider,home,assistant
        self.agents={}
        self.agenda=None
        self.hermes=MemberHermes(members,household_runtime=household_runtime)
    def agent(self,principal):
        with self.members.lock:
            self.members.current(principal)
            if principal.nonce not in self.agents:
                provider=PersonalProvider(self.provider,self.hermes,principal)
                agent=EchoAgent(MemberSettings(self.store,self.members,principal.member),provider,self.members.memory(principal))
    ...
    def respond(self,text,principal,lookup=False,*,before,**kwargs):
        self.members.current(principal)
        agent=self.agent(principal)
        kwargs['cancel']=PersonalCancellation(self.members,principal,before,self.store,kwargs.get('cancel'),agent.provider.stopped)
        ...
        result=agent.respond(text,principal,lookup,**kwargs)
        if kwargs['cancel'].is_set():
            return {'status':'unavailable','capability':'conversation','text':'Personal access changed or the request was cancelled. The reply was discarded.'}
        return result
    def clear(self,principal):
        agent=self.agents.pop(principal.nonce,None)
        if agent:
            agent.provider.stopped.set();agent.clear(str(principal))
