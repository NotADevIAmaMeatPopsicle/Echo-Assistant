...
(async()=>{const browser=await chromium.launch({channel:'chrome',headless:true});try{
 ...
 await page.goto('http://127.0.0.1:8788/display#settings');await page.locator('#source-load').click();
 ...
 const ring=await page.request.post('http://127.0.0.1:8788/v1/demo/doorbell',{data:{}});assert.equal(ring.status(),200);
