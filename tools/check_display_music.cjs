...
(async()=>{
  ...
  try{
    ...
    const cover=await page.locator('.album-art').boundingBox();
    assert.ok(cover.width>=220);assert.equal(await page.locator('.music-library,#video-provider-card').count(),0);
    assert.ok(await page.locator('#music-device-volume').isVisible());
    ...
    await page.locator('#tab-local').press('ArrowRight');assert.ok(await page.locator('#files-panel').isVisible());
    await page.locator('#tab-files').press('ArrowRight');assert.ok(await page.locator('#speakers-panel').isVisible());
