...
(async()=>{
  ...
  try{
    ...
    await page.addInitScript(()=>{HTMLMediaElement.prototype.play=()=>{throw Error('Audio forbidden in connection check');};});
    ...
    assert.match(await page.locator('#music-output-note').textContent(),/This display is connected/);
    ...
    console.log('Display/server, speaker, voice-service, pairing, and recovery states passed. No sound or device actions.');
