...
(async()=>{
 ...
 try{
  ...
  const fake=`window.fakeTracks=[];window.fakeRooms=[];window.fakeConnectOptions=[];window.deferCapture=false;window.resolveCapture=null;
  ...
  await page.route('**/*',route=>{
    ...
    if(url.pathname.startsWith('/v1/calling/')){
      ...
      return route.fulfill({json:{id:'a'.repeat(32),url:'wss://calls.example.com',token:'synthetic',expires_at:Date.now()/1000+900,invite_expires_at:Date.now()/1000+120,...(action==='start'?{code:'b'.repeat(32),private_transport:true}:{})}});
    }
  ...
  assert.deepEqual(await page.evaluate(()=>fakeConnectOptions[0].rtcConfig),{iceServers:[]},'Private mode must use only its server-provided ICE candidates');
  ...
  assert.equal(await page.evaluate(()=>Object.hasOwn(fakeConnectOptions.at(-1),'rtcConfig')),false,'Public/default mode must preserve LiveKit ICE configuration');
  assert.ok(await page.evaluate(()=>fakeConnectOptions.length>=2),'Both private start and public join must connect');
  ...
  assert.deepEqual(errors,[]);console.log('Calling: private start ICE override, public join default ICE, demo refusal, quiet audio, camera/mute, cancellation, invitation, access loss, settings redaction and phone layout passed.');
