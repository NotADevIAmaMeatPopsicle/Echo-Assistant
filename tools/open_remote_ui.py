...
def launch_url(settings=False,expected_mode='validation',display=False):
    ...
    return value['url'].replace('127.0.0.1','localhost')+('/display' if display else '/settings' if settings else '/')+'#ticket='+result.json()['ticket']
...
if __name__=='__main__':
    ...
    webbrowser.open(launch_url(args.settings,display=args.display))
