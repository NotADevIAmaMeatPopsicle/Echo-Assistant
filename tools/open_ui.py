...
def launch_url(settings=False):
    ...
    if target.exists():
        ...
        if mode=='remote':
            ...
            return remote_url(settings,expected_mode='device')
    ...
    with httpx.Client(base_url='http://127.0.0.1:8768', trust_env=False, timeout=5) as client:
        ...
        return 'http://127.0.0.1:8768'+('/settings' if settings else '/')+'#ticket='+response.json()['ticket']
