...
function scheduleDescription(item) {
  ...
  return `${item.time} · ${repeat} · ${item.timezone} · ${item.destination === 'round' || !item.destination ? 'Round speaker' : item.destination === data.displayAlerts?.destination ? 'This display' : 'Paired display'}`;
}
...
notificationForm.innerHTML='<span class="eyebrow">LEAVE A LITTLE NOTE</span><h2>Household message</h2><label>Title<input id="notice-title" maxlength="80" value="A note for home" required></label><label>Message<textarea id="notice-message" maxlength="400" rows="3" required></textarea></label><label class="check-label"><input id="notice-announce" type="checkbox">Also sound an alert on this device</label><p class="tiny soft">Messages appear in Notifications. The Pi plays a soft chime and shows the message; the round speaker reads it aloud. Alerts respect quiet hours and stay with the device that created them.</p><button class="pill primary" type="submit" data-requires="schedules">Send message</button>';
