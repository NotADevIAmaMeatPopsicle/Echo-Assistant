...
(()=>{
  ...
  function render(){
    ...
    $('video-provider-handoff').disabled=!ready;
    ...
    else if(!output?.available)status(output?.error||'Playback on this screen is unavailable. Continue on your phone.');
  ...
  async function save(event){
    ...
    const video_id=$('video-provider-id').value.trim(),allowed_display_ids=[...$('video-provider-displays').querySelectorAll('input:checked')].map(input=>input.value);
    if(video_id&&!validID(video_id)){$('video-provider-settings-status').textContent='Enter exactly 11 letters, numbers, underscores or hyphens.';return;}
  ...
  $('video-provider-handoff').onclick=()=>{if(identity!==stamp()||!selection?.available||!validID(selection.video_id))return;$('video-provider-url').value='https://www.youtube.com/watch?v='+selection.video_id;$('video-provider-copy-status').textContent='';dialog.showModal();};
