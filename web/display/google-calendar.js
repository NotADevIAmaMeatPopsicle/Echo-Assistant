...
(()=>{
  ...
  function renderAccounts(){
    $('google-accounts').innerHTML=(state.accounts||[]).map(a=>`<div class="google-account"><h3>${esc(a.label)}</h3><p class="tiny soft">${a.calendar_count} calendars found · ${a.write_access?'editing permission connected':'read-only connection'} · sharing is selected separately</p><div class="row"><button class="pill" type="button" data-google-sync="${esc(a.id)}">Refresh calendar list</button><button class="pill" type="button" data-google-disconnect="${esc(a.id)}">Disconnect</button></div></div>`).join('');
  }
  async function load(){
  ...
  $('google-connect').onsubmit=async e=>{
    ...
    try{
      const result=await api('/v1/calendar/google/flows',{label:$('google-label').value,client,write_access:$('google-write-access').checked});const url=new URL(result.url);
