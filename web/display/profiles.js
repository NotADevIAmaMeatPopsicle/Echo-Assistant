...
(()=>{
  ...
  $('display-access-form').onsubmit=async event=>{
    ...
    try{await api(target.id==='round'?'/v1/round/profile':'/v1/displays/'+target.id+'/profile',{revision:target.revision,profile},'PUT');if(version!==epoch)return;dialog.close();pairingListAt=0;miniAt=0;await loadPairedDisplays();await loadMiniAccess();toast('Device access saved.');}
