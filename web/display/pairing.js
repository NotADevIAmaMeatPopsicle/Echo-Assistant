...
let pairExpiry=0, pairingListLoading=false, pairingListAt=0;
async function loadPairedDisplays() {
  ...
  try {
    const result=await api('/v1/displays'); pairingListAt=Date.now();
    $('paired-displays').innerHTML=result.items.length ? result.items.map(d=>`<div class="schedule-item row spread"><div><strong>${esc(d.name)}</strong><p class="tiny soft">${d.last_seen ? 'Last seen '+esc(new Date(d.last_seen*1000).toLocaleString()) : 'No connection in this host session'}</p></div><button class="pill" data-revoke-display="${d.id}">Revoke</button></div>`).join('') : empty('No paired displays yet.');
