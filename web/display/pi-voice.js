...
function renderPiVoice(){
  ...
  if(signature!==piVoiceSignature&&!$('pi-voice-form').contains(document.activeElement)){
    ...
    $('pi-voice-volume').value=state.settings.volume;$('pi-voice-level').textContent=state.settings.volume+'%';
...
$('pi-voice-form').onsubmit=event=>{event.preventDefault();void action(()=>api('/v1/display/local-voice',{enabled:$('pi-voice-enabled').checked,muted:data.piVoice.settings.muted,input:$('pi-voice-input').value,output:$('pi-voice-output').value,volume:Number($('pi-voice-volume').value),echo_cancelled_input:$('pi-voice-aec').checked,allow_home:$('pi-voice-home').checked},'PUT'),'Pi voice settings saved. Use Unmute this Pi when ready to listen.');};
