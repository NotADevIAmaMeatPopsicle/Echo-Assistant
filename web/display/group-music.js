...
(()=>{
  ...
  $('group-music-form').onsubmit=event=>{event.preventDefault();if(config)action(()=>save({...config.config,enabled:$('group-music-enabled').checked,url:$('group-music-url').value.trim(),max_volume:Number($('group-music-max').value),receivers:[...$('group-music-receivers').querySelectorAll('input:checked')].map(el=>el.value)},$('group-music-token').value||null),'Music connection saved.');};
